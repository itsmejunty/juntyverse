...
import { ArrowRight, Sparkles, Star, Code, Palette, Zap, Rocket, Globe, Database, Cpu } from 'lucide-react';
...
const Hero = () => {
  ...
  const floatingIcons = [
    { Icon: Code, position: { top: '15%', left: '10%' }, delay: '0s', color: 'text-purple-600' },
    { Icon: Palette, position: { top: '25%', right: '15%' }, delay: '1s', color: 'text-violet-600' },
    { Icon: Zap, position: { bottom: '30%', left: '8%' }, delay: '2s', color: 'text-purple-700' },
    { Icon: Rocket, position: { top: '40%', right: '8%' }, delay: '0.5s', color: 'text-fuchsia-600' },
    { Icon: Globe, position: { bottom: '20%', right: '20%' }, delay: '1.5s', color: 'text-violet-700' },
    { Icon: Database, position: { top: '60%', left: '15%' }, delay: '2.5s', color: 'text-purple-800' },
    { Icon: Cpu, position: { bottom: '15%', left: '25%' }, delay: '3s', color: 'text-indigo-600' }
  ];
  ...
  return (
    <section className="min-h-screen pt-32 pb-16 relative overflow-hidden bg-gradient-to-br from-purple-50 via-white to-purple-100">
      ...
      {floatingIcons.map(({ Icon, position, delay, color }, index) => (
        <div
          key={index}
          className={`absolute animate-float opacity-40 hover:opacity-70 transition-opacity duration-300 ${color}`}
          style={{ 
            ...position, 
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col md:flex-row items-center gap-12 max-w-6xl mx-auto">
          ...
          <div className="relative mb-8 md:mb-0 order-1 md:order-2 animate-fade-in group">
            ...
            <div className="p-2 bg-gradient-to-br from-purple-500 via-violet-500 to-fuchsia-500 rounded-full transform group-hover:scale-105 transition-all duration-300 purple-glow hover:shadow-3xl">
              <Avatar className="w-52 h-52 border-4 border-white shadow-2xl group-hover:shadow-3xl transition-all duration-300 hover:rotate-3">
                <AvatarImage 
            ...
            {/* Enhanced Dynamic Star Decorations */}
            <span className="absolute top-0 -right-2 text-purple-600 animate-bounce hover:scale-125 transition-transform cursor-pointer">
              <Star className="h-5 w-5 fill-purple-200" />
            </span>
            <span className="absolute bottom-4 -left-4 text-violet-600 animate-bounce hover:scale-125 transition-transform cursor-pointer" style={{ animationDelay: '0.5s' }}>
              <Star className="h-6 w-6 fill-violet-200" />
            </span>
            <span className="absolute top-16 -left-6 text-fuchsia-500 animate-pulse hover:scale-125 transition-transform cursor-pointer">
              <Star className="h-4 w-4 fill-fuchsia-100" />
            </span>
            <span className="absolute -top-2 left-12 text-purple-500 animate-bounce hover:scale-125 transition-transform cursor-pointer" style={{ animationDelay: '1s' }}>
          ...
          <div className="flex flex-col items-start space-y-8 max-w-3xl order-2 md:order-1">
            ...
            <Badge 
              variant="outline" 
              className={`py-3 px-6 gap-3 text-sm glass-effect border-purple-200 hover:shadow-lg hover:shadow-purple-200/50 transition-all duration-300 hover:scale-105 cursor-pointer ${
            ...
            {/* Enhanced Typography with Typewriter Effect */}
            ...
            {/* Enhanced Description with Highlight Effects */}
            <p className={`text-lg md:text-xl text-muted-foreground/90 max-w-2xl leading-relaxed ${
              ...
              <span className="text-gradient-purple font-medium hover:font-semibold transition-all duration-300 cursor-pointer"> exceptional web experiences </span> 
              with React and Node.js. I build elegant, responsive solutions that combine 
              <span className="text-gradient-purple font-medium hover:font-semibold transition-all duration-300 cursor-pointer"> aesthetics with performance</span>.
            </p>
            ...
            <div className={`flex flex-wrap gap-3 pt-4 ${
              ...
              {['React', 'TypeScript', 'Node.js', 'Tailwind CSS', 'MongoDB'].map((skill, index) => (
                <Badge 
                  key={skill} 
                  className="px-4 py-2 bg-white/80 text-purple-700 hover:bg-purple-100 hover:scale-110 transition-all duration-300 cursor-pointer glass-effect"
        ...
        <div className="hidden md:flex absolute bottom-12 left-1/2 -translate-x-1/2 flex-col items-center animate-bounce cursor-pointer group hover:scale-110 transition-all duration-300">
          ...
          <div className="relative">
            ...
            <div className="absolute inset-0 bg-purple-400/20 rounded-full blur-sm group-hover:bg-purple-400/40 transition-all duration-300"></div>
          </div>
        </div>
      </div>
    </section>
